...
MII_data <- cbind(log2_NonCap$V3,log2_Cap$V3,log2_tail$V3)
colnames(MII_data) <- c("MII_body","MII_Cap","MII_tail")
...
library(umap)
umap_result <- umap::umap(pca_result$x, n_components = 2)
plot(umap_result$layout[,1], umap_result$layout[,2], asp = 1, xlab = "UMAP 1", ylab = "UMAP 2",cex=0.1,pch=20)
library(dbscan)

dbscan_result <- dbscan(umap_result$layout, eps = 0.5, minPts = 10)
